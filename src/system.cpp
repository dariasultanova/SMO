  ...
  step_by_step_mode_(Constants::step_by_step_mode())
{
  ...
  buffer_  = std::unique_ptr<Buffer>(new Buffer(Constants::buffer(), counter_));
...
void System::next_iteration() {
  ...
  if (process_rest_)
  {
    try {
      ...
      status_ = "buffer is empty, simulation is over";
...
int System::get_progress() const {
  double total = static_cast<double>((counter_->total()));

  int progress = static_cast<int>((std::round(total / max_requests_ * 100)));
