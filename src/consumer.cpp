...
void Consumer::process_request(Request::Request_ptr & request) {
  ...
  last_request_ = (std::to_string(request->get_priority() + 1) + "." + std::to_string(request->get_number()));
  counter_->add_processed(priority_);
}

void Consumer::next_time_point() {
  // exponential
  current_time_ += -(1.0 / Constants::lambda()) * std::log(1.0 - Constants::distribution());
}
