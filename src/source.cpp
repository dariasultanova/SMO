...
Request::Request_ptr Source::get_request()
{
  ...
  last_request_ = (std::to_string(priority_ + 1) + "." + std::to_string(++requests_num_));

  return std::make_shared<Request>(priority_, requests_num_, creation_time);
}

void Source::next_time_point()
{
  // equable
  current_time_ += Constants::distribution() * (Constants::beta() - Constants::alpha()) + Constants::alpha();
}
