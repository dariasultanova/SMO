...
{
  return std::accumulate(processed_.begin(), processed_.end(), 0);
}

unsigned Counter::rejected() const
{
  return std::accumulate(rejected_.begin(), rejected_.end(), 0);
}
...
{
  if (total_.at(i) != 0)
    return 100 * (static_cast<double>(rejected_[i]) / total_[i]);
...
double Counter::get_rejection_probability() const
{
  ...
  for (const int rejected : rejected_) {
    if (total_.at(i) != 0) {
      sum += 100 * (static_cast<double>(rejected) / total_[i]);
    }
    ++i;
  }
...
double Counter::count_consumers_coeff(const index_t i, const time_t realisation_time) const
{
